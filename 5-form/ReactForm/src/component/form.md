  ...
  return (
    <form className="form" onSubmit={handleSubmit}>
      ...
      <div>
        ...
        <input
          type="text"
          //we can direct type=email for deafult formating 
          name="email"
...
const FormWithoutYup = () => {
  const fields = [
    ...
    { name: "birthDate", type: "date", label: "Date of Birth", placeholder: "Enter your date of birth" },
  ];

  const [formData, setFormData] = useState(
    fields.reduce((acc, field) => ({ ...acc, [field.name]: "" }), { gender: "", interests: [] })
  );
  ...
  const validateForm = () => {
    ...
    // Add validation for fields dynamically
    fields.forEach(({ name }) => {
      if (!formData[name]) {
